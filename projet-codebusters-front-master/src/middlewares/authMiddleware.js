...
const authMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case SUBMIT_LOGIN: {
      ...
      axios.post('http://127.0.0.1:8000/api/login', { username: email, password: password })
        .then((response) => {
    ...
    case SUBMIT_SIGNIN: {
      ...
      axios.post('http://127.0.0.1:8000//api/user/register', { nickname: nickname, email: email, password: password })
        .then(() => {
    ...
    case SUBMIT_USER_UPDATE: {
      ...
      axios.patch('http://127.0.0.1:8000//api/user/update', { nickname: nickname, email: email },
        {
          headers: {
    ...
    case SUBMIT_USER_PASSWORD: {
      ...
      axios.patch('http://127.0.0.1:8000/api/user/update', { password: password },
        {
          headers: {
            Authorization: `Bearer ${store.getState().auth.token}`,
          },
    ...
    case SUBMIT_COMMENT: {
      ...
      axios.post('http://127.0.0.1:8000/api/comment/create', { content: comment, rating: rating, game: id },
        {
          headers: {
            Authorization: `Bearer ${store.getState().auth.token}`,
          },
    ...
    case GET_USER_SCORES: {
      store.dispatch(setLoading());
      axios.get('http://127.0.0.1:8000/api/user/read',
        {
          headers: {
            Authorization: `Bearer ${store.getState().auth.token}`,
          },
