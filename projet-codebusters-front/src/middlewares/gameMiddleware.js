...
const gameMiddleware = (store) => (next) => (action) => {
  switch (action.type) {
    case START_GAME: {
      ...
      axios.post('http://127.0.0.1:8000/api/game/create', { user: id },
        {
          headers: {
    ...
    case SUBMIT_ANSWER: {
      ...
      axios.post(`http://127.0.0.1:8000/api/game/answer/${id}`, { answer: inputGameValue },
        {
          headers: {
            Authorization: `Bearer ${store.getState().auth.token}`,
          },
    ...
    case END_GAME: {
      ...
      axios.put(`http://127.0.0.1:8000/api/game/update/${id}`, {},
        {
          headers: {
            Authorization: `Bearer ${store.getState().auth.token}`,
          },
